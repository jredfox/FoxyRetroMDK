...
{
    Copy-Item -Path "$PSScriptRoot\VerCheck.jar" -Destination "$mdk_dir\forge\VerCheck.jar" -Force | out-null
...
if ($patch_mcp723 -eq "T") {
    ...
    & "$mdk_dir\runtime\bin\python\python_mcp.exe" "$PSScriptRoot\replace.py" "$mcp_cmds" "classpath = [self.binclient] + self.cpathclient" "classpath = [self.binclient, self.srcclient] + self.cpathclient" "classpath = [self.binserver] + self.cpathserver" "classpath = [self.binclient, self.srcclient] + self.cpathserver"
}
