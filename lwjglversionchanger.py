...
if __name__ == "__main__":
    ...
    lwjgl_natives_windows = lwjgl_natives_base + "windows.jar"
    lwjgl_natives_osx = lwjgl_natives_base + "osx.jar"
    lwjgl_natives_linux = lwjgl_natives_base + "linux.jar"
    download_extract(lwjgl_url, lwjgl_jar, None)
    download_extract(lwjgl_util_url, lwjgl_util_jar, None)
    download_extract(lwjgl_natives_base + "windows", lwjgl_natives_windows_natives_jar, None)
    download_extract(lwjgl_natives_base + "osx", lwjgl_natives_macosx_natives_jar, None)
    download_extract(lwjgl_natives_base + "linux", lwjgl_natives_linux_natives_jar, None)

def download_extract(url, target, sha1):
    urllib.urlretrieve(url, target)
    downloaded_sha1 = get_sha1(target)
    if downloaded_sha1 != sha1 and (not sha1 is None):
