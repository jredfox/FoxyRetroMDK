...
function Patch-MDKPY {
    ...
    if [[  "$patch_applet" == "T" ]]; then
        ...
        cp -f "$SCRIPTPATH/patch_applet.py" "$mcp_dir/forge/patch_applet.py"
        cp -rf "$SCRIPTPATH/resources" "$mcp_dir/forge"
    fi
...
function Install-1.6x {
    ...
    ExitOnDLFail="false"
    Download "$temp/python_fml_2.7.9.zip" "$python_url" "true" "4"
    ExitOnDLFail="true"
